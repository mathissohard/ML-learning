...
class ActorCriticSolver(object):
    ...
    def train(self):
        ...
        true_loss_actor = self.loss_actor(valid_data_actor, training=False, cheat_value=True,
                                          cheat_control=True).numpy()
        # begin sgd iteration
        for step in range(self.net_config.num_iterations + 1):
            if step % self.net_config.logging_frequency == 0:
                loss_critic = self.loss_critic(valid_data_critic, training=False, cheat_control=False).numpy()
                loss_actor = self.loss_actor(valid_data_actor, training=False, cheat_value=False,
                                             cheat_control=False).numpy()
                ...
                training_history.append(
                    [step, loss_critic, loss_actor, err_value, err_value_infty, err_control, err_value_grad, err_cost,
                     elapsed_time])
                if self.net_config.verbose:
                    logging.info(
                        "step: %5u, loss_critic: %.4e, loss_actor: %.4e, err_value: %.4e, err_value_infty: %.4e, err_control: %.4e, err_value_grad: %.4e, err_cost: %.4e, elapsed time: %3u" % (
                            step, loss_critic, loss_actor, err_value, err_value_infty, err_control, err_value_grad,
                            err_cost, elapsed_time))
            if step % self.net_config.logging_frequency == 0:  #self.net_config.num_iterations:
                ...
                num_sample = np.shape(dw_sample)[0]
                x_smp, dt, coef = self.model_actor.propagate(
                    num_sample, x0, dw_sample, self.model_actor.NN_control, training=False,
                    T=self.eqn_config.total_time_actor,
                    N=self.eqn_config.num_time_interval_actor, cheat=False)
                #x = tf.reshape(x_smp[0, :, :], [self.eqn_config.dim, self.eqn_config.num_time_interval_critic + 1])
                print("state x:", x_smp)
                #print(x_smp.shape)
            ...
            if self.train_config.train == "actor-critic" or self.train_config.train == "actor":
                self.train_step_actor(self.sample(self.net_config.batch_size, self.eqn_config.num_time_interval_actor))
        return np.array(training_history), x0, y, true_y, z, true_z, grad_y, x_smp
    ...
    def loss_critic(self, inputs, training, cheat_control):
        ...
        #     tf.where(tf.abs(equa) < DELTA_CLIP, tf.square(equa), 2 * DELTA_CLIP * tf.abs(equa) - DELTA_CLIP ** 2))
...
class CriticModel(tf.keras.Model):
    ...
    def call(self, inputs, model_actor, training, cheat_control):
        ...
        x, dt, coef = self.propagate(num_sample, x0, dw, model_actor.NN_control, training,
                                     self.eqn_config.total_time_critic, self.eqn_config.num_time_interval_critic,
                                     cheat_control)
        for t in range(self.eqn_config.num_time_interval_critic):
            ...
            if self.train_config.TD_type == "TD1":
                ...
                delta_y_diffusion = tf.linalg.matvec(self.bsde.sigma(x[:, :, t], u, num_sample), dw[:, :, t])
                delta_y_diffusion = tf.reduce_sum(
                    delta_y_diffusion * self.NN_value_grad(x[:, :, t], training, need_grad=False), axis=1,
                    keepdims=True)
        ...
        # delta代表-TD1，delta_bdry代表边界惩罚：V(X)-g(X)
        delta = self.NN_value(x[:, :, 0], training, need_grad=False) - y - self.NN_value(x[:, :, -1], training,
                                                                                         need_grad=False) * discount
        a = tf.cast([[0.0], [0.0]], dtype=tf.float64)

        delta_bdry = x[:, :, -1] - tf.squeeze(tf.repeat(tf.expand_dims(a, 0), 1024, axis=0, name=None))
...
class ActorModel(tf.keras.Model):
    ...
    def call(self, inputs, model_critic, training, cheat_value, cheat_control):
        ...
        num_sample = np.shape(dw)[0]
        y = 0
        x, dt, coef = self.propagate(num_sample, x0, dw, self.NN_control, training, self.eqn_config.total_time_actor,
