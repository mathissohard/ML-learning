        ...
        for step in range(self.net_config.num_iterations+1):
            ...
            if step == self.net_config.num_iterations:
                ...
                x_smp, dt, coef = self.model_actor.propagate(
                    num_sample, x0, dw_sample, self.model_actor.NN_control, training=False, T=self.eqn_config.total_time_actor, N=self.eqn_config.num_time_interval_actor, cheat=False)
...
class CriticModel(tf.keras.Model):
    ...
    def call(self, inputs, model_actor, training, cheat_control):
        ...
        for t in range(self.eqn_config.num_time_interval_critic):
            ...
            w = self.bsde.w_tf(x[:, :, t], u, num_sample, N=self.eqn_config.num_time_interval_actor, T=self.eqn_config.total_time_actor) # running cost
            #integrand for drift
            ...
            if self.train_config.TD_type == "TD1":
                ...
                delta_y_diffusion = tf.linalg.matvec(self.bsde.sigma(x[:,:,t], u, num_sample), u)
                delta_y_diffusion = tf.reduce_sum(delta_y_diffusion * self.NN_value_grad(x[:,:,t], training, need_grad=False), axis=1, keepdims=True)
        ...
        a = tf.cast([[1.0], [0.0]], dtype=tf.float64)
...
class ActorModel(tf.keras.Model):
    ...
    def call(self, inputs, model_critic, training, cheat_value, cheat_control):
        ...
        for t in range(self.eqn_config.num_time_interval_actor):
            if cheat_control == False:
                w = self.bsde.w_tf(x[:,:,t], self.NN_control(x[:,:,t], training, need_grad=False), num_sample, N=self.eqn_config.num_time_interval_actor, T=self.eqn_config.total_time_actor)
            else:
                w = self.bsde.w_tf(x[:,:,t], self.bsde.u_true(x[:,:,t]), num_sample, N=self.eqn_config.num_time_interval_actor, T=self.eqn_config.total_time_actor)
        ...
        if cheat_value == False:
            ...
            delta_bdry = tf.where(tf.abs(delta_bdry) < DELTA_CLIP, tf.square(delta_bdry),
